...
export async function GET(request: NextRequest) {
  try {
    ...
    const balls = await query(``, overId ? [parseInt(overId)] : undefined);
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!over_id || ball_number === undefined || !batsman_id || !bowler_id || 
        runs_off_bat === undefined || is_legal_delivery === undefined) {
      return NextResponse.json(
        { error: 'over_id, ball_number, batsman_id, bowler_id, runs_off_bat, and is_legal_delivery are required' },
        { status: 400 }
      );
    }
