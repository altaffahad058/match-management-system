...
export async function GET(request: NextRequest) {
  try {
    ...
    const innings = await query(``, matchId ? [parseInt(matchId)] : undefined);
...
export async function POST(request: NextRequest) {
  try {
    ...
    const innings = await query(``, [parseInt(match_id), parseInt(batting_team_id), parseInt(bowling_team_id), parseInt(innings_number)]);
